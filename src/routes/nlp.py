...
async def index_project(request: Request, project_id: str, push_request: PushRequest):
    ...
    if not project:
        return JSONResponse(status_code= status.HTTP_400_BAD_REQUEST,
                            content={"signal": ResponseStatus.PROJECT_NOT_FOUND} )
    ...
    while has_records:
        ...
        idx += len(page_chunks)
        inserted = nlp_controller.index_into_vdb(project= project, chunks= page_chunks,chunks_ids= chunks_ids, do_reset= push_request.do_reset)

        if not inserted: 
            return JSONResponse(
                status_code= status.HTTP_400_BAD_REQUEST,
                content={"signal":  ResponseStatus.INSERT_INTO_VDB_ERROR})
        inserted_items_count += len(page_chunks)
        
    return JSONResponse(status_code= status.HTTP_200_OK,
                        content= {"signal": ResponseStatus.INSERT_INTO_VDB_SUCCESS,
...
async def get_project_index_info(request: Request, project_id):
    ...
    collection_info = nlp_controller.get_vdb_collection_info(project= project)

    return JSONResponse(
        status_code= status.HTTP_200_OK,
        content = {"signal": ResponseStatus.VECTORDB_COLLECTION_RETRIEVED,
